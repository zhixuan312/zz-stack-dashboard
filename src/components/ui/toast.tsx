...
interface ToastItem {
   ...
   *
   * Optional: every caller that passes nothing renders the default branch unchanged, which is
   * what makes "the other callers are unaffected" a property of the type rather than a hope.
   */
 ...
 * unbounded list grows past the top of the viewport — and the code that produces them is per-item
 * error handling: a failing SSE reconnect, or one toast per row of a bulk action, puts as many on
 * screen as there are failures. The oldest go, because the newest describes what just happened.
 */
const MAX_VISIBLE = 4;
...
export function showToast(toast: Omit<ToastItem, 'id'>): void {
  ...
  // Drop the overflow and its pending timer — a timer for a toast nobody can see would fire later
  // and re-render the stack for nothing.
...
/**
 * Not exported: every dismissal originates inside this module — the auto-dismiss timer, the card's
 * X, and the Retry button.
 *
 * DELIBERATE: `durationMs` stays although no production call site passes one. It is documented API
 * on `showToast`, one field wide, and a toast primitive without a duration override is the
 * unusual thing.
 */
function dismissToast(id: string): void {
  const t = timers.get(id);
  // Cancel the auto-dismiss: without this a manually-closed toast still wakes its timer later to
  // filter a list it is no longer in, re-rendering every toast on screen to do it.
...
/**
 * `useSyncExternalStore`, the same primitive `app-phase` and `stage-substeps` subscribe with. A
 * `useState` counter incremented from a listener would be a second way of doing the one job, and
 * the one React does not guarantee against tearing under concurrent rendering.
 */
