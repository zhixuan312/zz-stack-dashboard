 ...
 * They are the same kind of thing, so both pages carry both halves and differ only in
 * their identity rail.
 *
                ...
                >
                  <Table>
                    ...
                    <TableBody>
                      {d.dimensions.map((x) => (
                        <TableRow key={x.name}>
                          <TableCell className="w-[22%] font-medium text-ink">{x.name}</TableCell>
                    ...
                    <TableBody>
                      {d.dimensions.map((x) => (
                        <TableRow key={x.name}>
                          ...
                          <TableCell className="text-right tabular-nums text-xs">{x.sd?.toFixed(2) ?? '—'}</TableCell>
                          <TableCell className="text-right tabular-nums text-xs">{x.n}</TableCell>
                          <TableCell className="text-right tabular-nums text-xs text-[var(--rose-deep)]">{x.low || '—'}</TableCell>
                          <TableCell className="text-right tabular-nums text-xs text-[var(--sage-deep)]">{x.high}</TableCell>
                        </TableRow>
                      ))}
                    </TableBody>
                  </Table>
                    ...
                    <TableBody>
                      {d.findings.map((f, i) => (
                        <TableRow key={i}>
                          ...
                          <TableCell className="max-w-[70ch] text-[13px] text-ink">{f.pattern}</TableCell>
