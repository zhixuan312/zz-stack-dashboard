// The brand is drawn in one place: AppMark. Everything below enumerates what that means.
import { readFileSync, existsSync, readdirSync } from 'node:fs';
let code = 0;
const mark = readFileSync('src/components/AppMark.tsx', 'utf8');
/* The src attribute, not the word. `/wordmark/` also matches this component's own docstring, so
 * AppMark could stop rendering the mark entirely and still pass. */
...
/* The rule: the brand is drawn in one place. Two things follow, both enumerable. A brand raster
 * is referenced only by AppMark. And an inline `<svg>` literal — the way you draw a mark by hand —
 * appears in exactly two files, neither of them a logo: the rail's Sparkle (one piece of ornament,
 * by decision) and TrendChart (a chart is SVG; that is not a mark). Anything else is a second
 * mark, whatever shape it takes.
 *
 * DELIBERATE: the allowlist is what makes the next second mark fail. Naming the hexagon and the
 * hand-drawn ZZ monogram catches history — a fresh `<svg><polygon>` logo in the rail beside
 * AppMark passes.
 */
const MARK_FILE = 'src/components/AppMark.tsx';
/* A count, not a membership test. A file-level allowlist lets a second hand-drawn mark be added
 * to a file already allowlisted for its one Sparkle. The numbers are the ones that exist: the
 * rail's single Sparkle, and TrendChart's two. Raising one is a decision somebody makes. */
