...
export default function PluginPage({ params }: { params: Promise<{ plugin: string }> }) {
  ...
  return (
    <DashboardPage
      ...
      // once, where the rest of what this plugin IS already lives.
      showPeriod={false}
      updatedAt={new Date()}
      scroll="inner"
      metrics={
      ...
      // WHAT THE PLUGIN IS in the rail; what its skills DID in the main column — the same
      // division as every other detail page on this console.
      rail={
        p ? (
          <div className="flex flex-col gap-4">
            <Panel title="What this plugin is">
              <dl className="flex flex-col gap-3 text-[13px]">
                ...
                />
                {p.owner ? <Row k="Owner" v={<span className="font-mono text-xs">{p.owner}</span>} /> : null}
            ...
            >
              <div className="h-full overflow-y-auto">
                <Table>
                  ...
                  <TableBody>
                    {/* STAGES FIRST AND IN ORDER, then everything else the plugin ships.
                        `stages` is the running order and the rest have none, so mixing them
                        alphabetically would put step 6 above step 2. */}
                    {[...p.skills]
                      .sort((a, b) =>
