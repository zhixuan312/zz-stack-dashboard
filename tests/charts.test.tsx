 ...
 * chart throws `must be used within TooltipProvider`.
 */
...
describe('TrendChart', () => {
  ...
  /* The boundary, not the fill. A volume bar is painted faint so it does not compete with
   * the area series above it, and a pastel fill at 20% on cream is absent — so it carries a
   * full-strength stroke in the same hue, the SVG spelling of `--chart-edge`.
   *
   * Asserted here rather than in a screenshot because no page passes `shape: 'bar'` today,
   * so a visual harness proves nothing about the path.
   *
