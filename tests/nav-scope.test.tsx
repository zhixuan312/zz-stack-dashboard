 ...
 * answer here — mounting the rail would drag in a router and a `/me` fetch to
 * ask the same thing less directly.
 */
...
describe('the rail in team mode', () => {
  ...
  it('does not call the surviving group "Platform"', () => {
    // Knowledge and People arrive scoped to the caller's own team. Leaving the
    // fleet's eyebrow over them names the wrong owner on the one view whose job
    // is to show a member their boundary.
  ...
  it('drops no section entirely, so nothing is left headless', () => {
    // A section filtered down to zero items would render as a bare eyebrow with
    // nothing under it. `navSections` removes it; today none actually empties,
    // and this asserts the filter kept every group that still has links.
...
describe('the rail is not headed by a word that says nothing', () => {
  it('gives Settings no eyebrow of its own', () => {
    // "YOU" over a single item called Settings said nothing the word Settings
    // did not, and a one-item group under a heading reads as a group that
